...
UCNAME="ctr-patient"
# pharmaledger at ctr-dev-pl
USERATHOST="pharmaledger@192.168.13.106"
...
#docker run --detach --hostname $UCNAME --network="host" --publish 8080:8080 --name $UCNAME --restart always pharmaledger/$UCNAME
docker run --detach --hostname $UCNAME --network="host" --publish 8080:8080 --mount source=external-volume,target=/ctr-workspace/ctr-patient/apihub-root/external-volume --name $UCNAME --restart always pharmaledger/$UCNAME
